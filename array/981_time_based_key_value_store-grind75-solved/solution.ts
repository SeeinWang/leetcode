...
class TimeMap {
    timeStampMap: Map<string, Array<{ timeStamp: number, value: string }>>;
    constructor() {
        this.timeStampMap = new Map()
    }

    set(key: string, value: string, timestamp: number): void {
        if (!this.timeStampMap.has(key)) {
            this.timeStampMap.set(key, []);
        }
        this.timeStampMap.get(key)!.push({ timeStamp: timestamp, value });
    }
